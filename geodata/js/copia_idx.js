...
//++++++++++++++++++++SLC ENTIDAD, MUNICIPIO, LOCALIDAD REFERENCIA++++++++++++++++++++++++++++++++++++
  //  $("#slc_estado_referencia").change(function(){
  ...
  //               $("#slc_municipio_referencia").empty();
  //               $("#slc_municipio_referencia").append("<option value=-1>Elija un Municipio</option>");
  //               //$("#slc_municipio_referencia").append("<option value=0>Toda la entidad</option>");
  //               $("#slc_municipio_referencia").append(data);
  ...
  //  $("#slc_municipio_referencia").change(function(){

  //           var valor=$("#slc_estado_referencia").val()+"\:"+$(this).val()+"\:";

  //           $.ajax({
  //             url:'zonas.php',
  //             type:'post',
  //             data: {"localidad": valor},
  //             dataType:'JSON',
  //             success: function(data){

  //               $("#slc_zona_referencia").empty();
  //               $("#slc_zona_referencia").append("<option value=-1>Elija una zona</option>");
  //               //$("#slc_zona_referencia").append("<option value=0>Todas las zonas</option>");
  //               $("#slc_zona_referencia").append(data);
  //               for (var i = 0; i < data.length; i++) {
                  
  //                  if(i==0){
  //                    $("#slc_zona_referencia").append("<option>"+data[i].territorio[3]+"</option>");
  //                  }//no se hace nada en la primera vuelta
  //                  else
  //                  {
  //                    if(data[i-1].territorio[3]===data[i].territorio[3]){
  ...
  //   $("#slc_zona_referencia").change(function(){
  //    var valor="condicion="+$("#slc_estado_referencia").val()+"\:"+$("#slc_municipio_referencia").val()+"%%"+"\:"+$(this).val();
  //    verPoligonos("crear",valor,"zona");
  //  })
...
//++++++++++++++++++++ ++++++++++++++++++++++++++++++++++++++++    
    function verPoligonos(selector,datos,valor_tipo_polig) {
        ...
        if (selector=="ver") 
        {
          // if ($("#chk_ver").is(":checked")) 
          //   { //no reinicia el mapa para que no sea acumulativo

          // }
          // else{
          //  initMap(); // reinicia el mapa para que no se acumulativo
          // }
          ...
          if(valor_tipo_polig=="ver"){
            ...
            $.ajax({
                        ...
                        success: function(data){
                            ...
                            //                       '<div id="siteNotice">'+
                            //                       '</div>'+
                            //                       '<h1 id="firstHeading" class="firstHeading text-primary">'+data[i].nombre+'</h1>'+
                            //                       '<div id="bodyContent">'+
                            //                           '<p><b>Asignacion <b/>Ninguna</p>'+
                            //                           '<br><p>Secciones: '+data[i].secciones+'</p>'+
                            ...
                            //                   '<div id="siteNotice">'+
                            //                   '</div>'+
                            //                   '<h3 id="firstHeading" class="firstHeading text-primary">yyy</h3>'+
                            //                   '<div id="bodyContent">'+
                            //                       '<p><b>Dirección: <b/>fff: '+i+'</p>'+
                            //                       '<p>Colonia: ddd'+i+'</p>'+
          ...
          //#################################
          if(valor_tipo_polig=="ver_secciones"){
              if ($("#chk_ver").is(":checked")) { //no reinicia el mapa para que no sea acumulativo
                ...
                $.ajax({
                  url:'resources/control.php', 
                  type:  'post',
                  data: {clase:'crud_poligonos',selector:'ver_secciones',data:datos},
                  dataType:'JSON',
                  success: function(data){
                     
                    var arreglo_poligonos_secciones=[];
                    var poligonos2=[];
                    var centro_poligono2;
                    $.each(data, function(i, value) { 
                      poligonos2=[];
                      ...
                      $.each(data[i].coords, function(key, value2) { 
                       var lati=parseFloat(data[i].coords[key][0]);
                       var lngi=parseFloat(data[i].coords[key][1]);
                       poligonos2.push({lat:lati,lng:lngi});
                      });
                      var polygon_center = new google.maps.Polygon({ //poligonos2 temporales para obtener su centro
                           paths: poligonos2,                        
                      ...
                      arreglo_poligonos_secciones.push(poligonos2);
                      var area = google.maps.geometry.spherical.computeArea(polygon_center.getPath());
                      polygon_center.setMap(map);
                      centro_poligono2= polygonCenter(polygon_center);

                      var marker = new google.maps.Marker({
                        position: centro_poligono2,
                        map: map,
                        title:data[i].nombre
                      });
                      var infowindow = new google.maps.InfoWindow({
                            ...
                            return function () {
                              infowindow.setContent(
                                        '<div id="content">'+
                                          '<div id="siteNotice">'+
                                          '</div>'+
                                         '<h3 id="firstHeading" class="firstHeading text-primary"> Nombre: '+data[i].nombre+'</h3>'+
                                          '<div id="bodyContent">'+
                                              '<p><b>Zona: <b/>'+data[i].zona+'</p>'+
                                              '<p><b>Área: <b/>'+area.toFixed(2)+'m2</p>'+
                                              '<p><b>Observaciones: <b/>'+data[i].observaciones+'</p>'+
                                              '<p><b>Fecha de creación: <b/>'+data[i].fecha_registro+'</p>'+
                                          '</div>'+
                                        '</div>'                                  
                    ...
                    map.setCenter(centro_poligono2);
                    map.setZoom(15);
                    },//fin de success  
          ...
          } //fin if ver_secciones 



          if(valor_tipo_polig=="zonas"){
              initMap();
              $.ajax({
                url:'resources/control.php', 
                type:  'post',
                data: {clase:'crud_poligonos',selector:'ver_zonas',data:datos},
                dataType:'JSON',
                success: function(data){
                    if(valor_tipo_polig=="zonas"){
                            ...
                            return function () {
                              infowindow.setContent(
                                        '<div id="content">'+
                                          '<div id="siteNotice">'+
                                          '</div>'+
                                         '<h3 id="firstHeading" class="firstHeading text-primary">'+data[i].nombre+'</h3>'+
                                          '<div id="bodyContent">'+
                                              '<p><span class="text-primary"><b>Zona: <b/></span>'+data[i].zona+'</p>'+
